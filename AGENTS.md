...
PathLab Forge is a standalone Windows/macOS desktop application. It performs computationally expensive WSI work locally and connects to PathLab Viewer only through a versioned API, resumable tus upload and the `.plslide` package.

The separate `PathLab-Viewer` repository owns server authentication, package import, private preview, publication and static tile delivery.
...
5. `docs/compatibility/OS_MATRIX.md` when platform behavior is involved
...
- server database or deployment
- public tile serving
- student/teacher workflows
- annotations or collaboration
- AI diagnosis
...
- Use bounded caches, streaming/tiled I/O and cancellable operations.
