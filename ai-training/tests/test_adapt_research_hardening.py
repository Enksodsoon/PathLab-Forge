...
def evidence_registry() -> dict[str, object]:
    return EvidenceRegistry((EvidenceRecord(
        ...
        "faculty-reviewer", "2026-08-02T12:00:00Z",
    ),)).freeze()
...
def test_claim_gate_blocks_positive_and_causal_variants(tmp_path: Path, claim: str) -> None:
    with pytest.raises(ValueError, match="unsupported wording"):
        render_manuscript(snapshot(tmp_path), evidence_registry(), analyze_normal_use((), ()), novelty=complete_novelty(), contribution_claim=claim)


def test_citation_must_use_record_allowed_wording(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="allowed wording"):
        render_manuscript(
            snapshot(tmp_path), evidence_registry(), analyze_normal_use((), ()), novelty=complete_novelty(),
            contribution_claim="Viewport behavior was documented [prior-vmat].",
        )
    text = render_manuscript(
        snapshot(tmp_path), evidence_registry(), analyze_normal_use((), ()), novelty=complete_novelty(),
        contribution_claim="An association was reported [prior-vmat].",
    )
...
def test_safe_ai_sequence_requires_signed_true_claim_and_unrelated_source() -> None:
    claim = EvidenceRecord("truth", "PMID:26110095", "abstract:result", "observational", "factual", "faculty", "2026-08-02", claim_text="Faculty-approved true claim", truth_status="verified_true")
    unrelated = SourceApproval("source-2", "unrelated", "faculty", "2026-08-02", "approved")
