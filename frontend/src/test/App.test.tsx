...
test('updates dimensions and file size live while drawing and reshaping a crop', async () => {
  ...
  expect(screen.getByText(/Estimated direct OME-TIFF ≈/)).toBeVisible()
  ...
  const drawnEstimate = screen.getByText(/Estimated direct OME-TIFF ≈/).textContent
  ...
  expect(screen.getByText(/Estimated direct OME-TIFF ≈/).textContent).not.toBe(drawnEstimate)
...
test('uses direct OME for new conversions and keeps direct results readable', async () => {
...
test('replaces the estimate with compact DZI size and quality evidence after conversion', async () => {
  ...
  expect(await screen.findByText('Estimated direct OME-TIFF ≈ 23.8 MB')).toBeVisible()
