...
if ($LASTEXITCODE -ne 0) { throw 'The 2.1.6 distribution did not pass its build and tests.' }

if (-not $OutputPath) {
    $OutputPath = Join-Path $repository 'build\Upgrade-PathLab-EvidenceMentor-2.1.6.ps1'
}
...
$content = @"
`$ErrorActionPreference = 'Stop'
& '$($installer.Replace("'", "''"))' -Action Upgrade -Version '2.1.6' ```
