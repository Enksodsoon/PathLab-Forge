...
param(
    ...
    [string] $CampaignId = 'monusac-od-watershed-heldout-20260824-v1',
...
$requestPath = Join-Path $campaignRoot 'request-cell-od-watershed-v2.json'
$attestationPath = Join-Path $campaignRoot 'attestation-cell-od-watershed-v2.json'
$packSource = Join-Path $repository 'src\main\resources\evidence-packs\cell-od-watershed-v2.json'
$protocolPath = Join-Path $repository 'qualification-protocols\cell-od-watershed-v2.json'
...
foreach ($required in @($packSource,$protocolPath,(Join-Path $state 'endpoint.json'),(Join-Path $state 'ipc-token'))) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "Required qualification input is unavailable: $required" }
}
$endpoint = Get-Content -LiteralPath (Join-Path $state 'endpoint.json') -Raw | ConvertFrom-Json
if ([version]$endpoint.serviceVersion -lt [version]'2.1.6') {
    throw 'Service 2.1.6 is required for bounded held-out cell-instance qualification.'
}
...
if ($status.queue.active -ne 0) { throw 'A worker is active; MoNuSAC staging stopped safely.' }

$derivedAcl = Get-Acl -LiteralPath $derivedRoot
$derivedSddl = $derivedAcl.Sddl
try {
    $currentIdentity = [Security.Principal.WindowsIdentity]::GetCurrent().Name
    $temporaryModify = New-Object Security.AccessControl.FileSystemAccessRule(
        $currentIdentity, 'Modify', 'ContainerInherit,ObjectInherit', 'None', 'Allow')
    $derivedAcl.AddAccessRule($temporaryModify) | Out-Null
    Set-Acl -LiteralPath $derivedRoot -AclObject $derivedAcl
    & (Join-Path $repository 'scripts\build-monusac-cell-cohort.ps1') `
        -StateRoot $state -CohortId $CohortId -DerivedUsedBytes ([long]$status.quota.derived.usedBytes)
...
try {
    ...
    $packTarget = Join-Path $campaignRoot 'cell-od-watershed-v2.json'
    ...
    if ($pack.packId -ne 'cell-od-watershed-v2' -or
            $pack.qualificationPolicy.protocolSha256 -ne $protocolSha) {
        throw 'The cell pack is not bound to the frozen MoNuSAC protocol.'
    }
    ...
    Write-JsonAtomic $campaignPath ([ordered]@{
        ...
        tracks=@([ordered]@{
            id='cell-od-watershed-monusac-v1';candidateId='cell-od-watershed-v2'
            capability='cell-morphology';scope='deployment';requestPath=[IO.Path]::GetFileName($requestPath)
...
[pscustomobject]@{
    ...
    SampleCount=23;CohortManifestSha256=$cohortSha;PermittedUse='private-research-restricted'
}
