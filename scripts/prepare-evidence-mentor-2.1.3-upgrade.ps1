...
if (-not $JavaHome) {
    $candidate = Get-ChildItem -LiteralPath (Join-Path $env:USERPROFILE '.gradle\jdks') `
        -Filter 'java.exe' -File -Recurse -ErrorAction SilentlyContinue |
        Where-Object { $_.FullName -match '17' } | Select-Object -First 1
...
if ($LASTEXITCODE -ne 0) { throw 'The 2.1.3 distribution did not pass its build and tests.' }

if (-not $OutputPath) {
    $OutputPath = Join-Path $repository 'build\Upgrade-PathLab-EvidenceMentor-2.1.3.ps1'
}
...
$content = @"
`$ErrorActionPreference = 'Stop'
& '$($installer.Replace("'", "''"))' -Action Upgrade -Version '2.1.3' ```
...
Write-Host 'Run it once from an Administrator PowerShell after the firewall provider recovers.'
